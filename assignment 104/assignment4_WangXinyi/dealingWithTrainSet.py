...
def dataSplit():
    ...
    for i in range(0,len(finalTS)):
        ...
        if(deltaday.days>=0):
            ...
            dayAfterPublished[i]=-1#feature dayAfterPublished
        if (1<=finalTS[i][2]-finalTS[i][3]<=10):
            ...
            artistRecentPublished.append([finalTS[i+1][1],finalTS[i+1][2]])#can't write this way cause even after the song was published on 6.18, it wasn't played everyday after 6.18,for example on,6.23 and 6.24 it wasn't played,so we have to manually add the date on
    ...
    #new feature hasRecentlypublished    
    ...
    for num in range(0,len(deleteList1)):
        ...
        #then I used finalTS=np.delete(finalTS,deleteList[0],0),which was also wrong,cause the file was orderd by songId not artistID,thus artists are not continuous
    ...
    uniqueUser1.to_csv('./uniqueU1_trainSet.csv',header=False,index=False)
    ...
    uniqueUser2.to_csv('./uniqueU2_trainSet.csv',header=False,index=False)
    ...
    uniqueUser3.to_csv('./uniqueU3_trainSet.csv',header=False,index=False)
    ...
    uniqueUser4.to_csv('./uniqueU4_trainSet.csv',header=False,index=False)

    ##output final trainSet
    ...
    finalTS.to_csv('./finalTrainSetFirstTrial.csv',header=False,index=False)
